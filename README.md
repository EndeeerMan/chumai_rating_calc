...
- 提供独立登录、注册与找回密码页面，支持邮箱验证码重置密码、换绑邮箱、修改密码及注销账号
...
| 地址 | 内容 |
| --- | --- |
| `/login.html` | 登录账号；成功后默认进入舞萌 DX 首页 |
| `/register.html` | 验证邮箱并创建账号 |
| `/forgot-password.html` | 通过已绑定邮箱的验证码重置密码 |
| `/` | 舞萌 DX 分数构成、成绩列表和游玩记录 |
| `/chunithm.html` | 中二节奏分数构成、成绩列表和游玩记录 |
| `/sync.html` | 微信公众号成绩同步 |
| `/profile.html` | 头像、昵称、邮箱、密码、背景和账号管理 |

认证页面允许未登录访问；成绩、同步和个人资料页面必须登录。未登录访问业务页面时会跳转
到 `/login.html`。旧账号尚未绑定邮箱时会先强制进入邮箱绑定页。

新用户名须为 `3–18` 位，只能包含 ASCII 字母、数字和下划线（`[A-Za-z0-9_]`）。新注册、
修改及重置密码均须为 `6–32` 位，只允许 ASCII 十进制 `33–126`（`!` 到 `~`，不含空格）。
这些限制适用于新用户名和新密码；已有账号仍可使用原用户名与原密码登录。

## 配置邮箱验证

注册、邮箱换绑和通过邮箱找回密码需要 SMTP。复制示例文件：
...
注册、换绑和找回密码的验证码均为 6 位数字，10 分钟内有效；同一邮箱重新发送需要等待
2 分钟。密码重置成功后，该账号的全部旧登录会话都会失效。
